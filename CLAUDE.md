...
- `src/db.js` — opens SQLite (WAL, foreign keys ON), creates the schema idempotently. `users`, `credentials` (passkeys, `ON DELETE CASCADE` from credentials to users), and `stat_counters` (one accumulated count per anonymous event type). `migrateLegacyEvents` upgrades the old append-only `events` table into counters on open.
...
- `src/routes/stats.js` — public `GET /api/usage`, mounted **before** the login rate limiter in `src/app.js` (so reading stats doesn't burn the login attempt budget). Returns only `totals`: the accumulated lifetime count per event type. Deliberately **no** current-state snapshot (active account count, how many currently use 2FA/passkeys) and **no** time-series/`last24h` — only counts of things that *happened*, as running totals. (Files/symbols are named `stats.*` internally; only the public URLs use `usage` — the `/stats` path was taken by an external system.)
...
**Login is two-step when 2FA is on.** `POST /api/login` verifies the (client-hashed) password with `argon2.verify`; if `totp_enabled`, it sets `req.session.pendingUserId` and returns `{ twofa: true }` *without* logging in. The session only becomes authenticated (`req.session.userId`) after `POST /api/login/totp` verifies the code. Password failures return a generic error; an unknown username still triggers a verify against a fixed dummy argon2 hash (`DUMMY_HASH` in `src/routes/auth.js`) so timing doesn't leak existence.

**Authentication always goes through `establishSession(req, userId)`** (`src/middleware.js`) — used by register, password login, the TOTP step, and passkey login. It calls `req.session.regenerate()` (new session ID on every privilege elevation → anti session-fixation; this also clears `pendingUserId`/`authChallenge`) then sets `userId` and `save()`s before responding. Don't set `req.session.userId` directly; use the helper.
...
**Stats are anonymous by construction and don't grow the DB.** Usage stats live in `stat_counters` — one row per event type holding a running `count` (no per-event rows, no timestamps, no user id/username/credential). Routes call `store.recordEvent(db, store.EVENTS.X)` (an UPSERT `count = count + 1`) at the relevant success/failure points (register, login success by method, failed login by method, logout, manual delete, cleanup prune, 2FA on/off, passkey add/remove); cleanup uses `recordEventBatch` for the count it removed. Recording is best-effort (swallows errors) so it can never break an auth flow. Because it's fixed-size counters there's nothing to prune. (An earlier append-only `events` table is auto-migrated into these counters and dropped on open — see `migrateLegacyEvents` in `src/db.js`.) The `/usage` page (`public/stats.html` + `public/js/stats.js`) is public and shows only these totals plus a client-computed login success rate. **Don't add anything user-identifying or per-event** — totals only.
...
When **hosted** behind a real domain, serve over **HTTPS** and override `RP_ID` (hostname only, e.g. `app.example.com`) and `ORIGIN` (full URL, e.g. `https://app.example.com`) to match — otherwise WebAuthn verification fails.

**Hosting hardening keys off the ORIGIN scheme, not `NODE_ENV`** (see `src/config.js`): when `ORIGIN` starts with `https://`, the session cookie gets `Secure`, HSTS is enabled, and `trust proxy` defaults to `1` (so the rate limiter sees the real client IP behind a reverse proxy). With an `http://` origin (localhost) these stay off so login works over plain HTTP — *even if `NODE_ENV=production`* (which `.env.example` ships). Override explicitly with `SECURE_COOKIES` and `TRUST_PROXY` if needed. This deliberately does **not** use `NODE_ENV`, because tying secure cookies to it would break `http://localhost` for the shipped `.env`.
